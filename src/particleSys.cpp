...
void particleSys::startUpdate(vec3 point)
{
  start.x = point.x;
  start.y = point.y;
  start.z = point.z;
...
void particleSys::gpuSetup() {
  ...
  cout << "start: " << start.x << " " << start.y << " " <<start.z << endl;
	for (int i=0; i < numP; i++) {
		...
		particle->load(start, i);
    vec4 particleColor = particle->getColor();
    pointColors[i*4+0] = particleColor.r;
		pointColors[i*4+1] = particleColor.g;
		pointColors[i*4+2] = particleColor.b;
		pointColors[i*4+3] = particleColor.a;
	}
...
void particleSys::update() {
  ...
  srand(1234);
  float r1, g1, b1, randF;
  randF = randf(0.80f,0.95f); 
